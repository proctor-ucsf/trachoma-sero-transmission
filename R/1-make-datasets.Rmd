---
title: "Trachoma endgame serology: create analysis datasets"
...
This script will be provided along with replication files for transparency but will not be able to be run. It creates the publicly available datasets that will be provided with the analysis scripts.  It creates two files: an individual level dataset `trachoma_sero_transmission_analysis_indiv.csv/.rds` and a cluster level dataset `trachoma_sero_transmission_analysis_cluster.csv/.rds`.  Note that the study includes some PCR and TF results that were only available at the cluster level (not individual level due to pooling) and so the cluster level dataset includes more children for PCR and TF indicators. All Pgp3 antibody data are available at the individual level.
...
Read in the base trachoma serology harmonized datasets (individual and cluster level)
...
# ID variables
#---------------------------------
ind_df <- read_rds(here(final_data_path,"trachoma_serology_public_data_indiv_v2.rds")) %>%
...
# ID variables
#---------------------------------
clu_df <- read_rds(here(final_data_path,"trachoma_serology_public_data_cluster_v2.rds")) %>%
  rename(cluster_id = cluster_id_public) %>%
  # filter to included studies
  filter(study_id %in% c("Kongwa2013","Kongwa2018","Malawi2014","MORDOR2015","Morocco2019","PRET2013","TAITU2018","TCC-Ethiopia2017","WUHA2016")) %>%
  mutate(location_year_name = droplevels(location_year_name)) 
...
#---------------------------------
clu_df4 <- clu_df3 %>%
  mutate(pgp3_minobs = ifelse(pgp3_n_tested >= minobs,TRUE,FALSE),
...
#---------------------------------
length(unique(ind_df2$cluster_id[ind_df2$pgp3_minobs]))
nrow(clu_df5 %>% filter(pgp3_minobs))
length(unique(ind_df2$cluster_id[ind_df2$pgp3_minobs])) == nrow(clu_df5 %>% filter(pgp3_minobs))
...
#---------------------------------
sum(!is.na(ind_df2$pgp3_bin[ind_df2$pgp3_minobs]))
sum(clu_df5$pgp3_n_tested[clu_df5$pgp3_minobs], na.rm=TRUE)
sum(!is.na(ind_df2$pgp3_bin[ind_df2$pgp3_minobs])) == sum(clu_df5$pgp3_n_tested[clu_df5$pgp3_minobs], na.rm=TRUE)
...
n_df <- ind_df2 %>% 
  filter(!is.na(pgp3_bin) & pgp3_minobs) %>% 
...
m_df <- ind_df2 %>% 
  filter(pgp3_minobs) %>%
  group_by(location_year_name, cluster_id) %>% 
  summarise(pgp3_n = sum(!is.na(pgp3_bin))) %>% 
...
study_df <- ind_df2 %>% 
  ...
  mutate(location_year_name_nm = ifelse(location_name %in% c("Wag Hemra (TAITU), Ethiopia", "Dosso, Niger (MORDOR)", "Matameye, Niger (PRET)"),
                                        paste0(location_year_name_nm, " †"), location_year_name_nm)) %>% 
