...
cutoff_df <- ind_df %>% 
  distinct(location_year_name, cutoff_orig = pgp3_cutoff) %>% 
...
ind_df_co <- ind_df %>%
  ...
  dplyr::select(individual_id, cluster_id, location_year_name, age_years,
                pgp3_cont, pgp3_nonneg, pgp3_bin, pgp3_cutoff,
                cutoff, cutoff_type) %>% 
  mutate(pgp3_bin_co = as.numeric(pgp3_cont>cutoff))
```
...
sp_co <- ind_df_co %>% 
  ...
  summarise(pgp3_n_pos_co = sum(pgp3_bin_co, na.rm = TRUE),
            pgp3_n_tested_co = sum(!is.na(pgp3_bin)),
            .groups = "drop") %>% 
...
get_scr <- function(curr_lyn, curr_co, curr_clu){
    ...
    glm_fit <- tryCatch(glm(pgp3_bin_co~1, offset = log(age_years),
                          data = temp_df,
...
cutoff_fig <- ind_df_co %>%
  ...
  geom_histogram(data = . %>% filter(cutoff_type == "orig"),
                 aes(x = pgp3_nonneg, fill = location_year_name,
...
if(save_figs) {
  ggsave(filename = here::here("output", "figS7_cutoff_fig.png"),
       combined_fig,
