...
# count positives out of total
clu_all %>% filter(location_year_name %in% lyn_all_list, pcr_n_tested>= min_clu_n) %>% nrow()
clu_all %>% filter(location_year_name %in% lyn_all_list, pcr_n_tested>= min_clu_n, pcr_prev==0) %>% nrow()
clu_all %>% filter(location_year_name %in% lyn_all_list, pcr_n_tested>= min_clu_n, pcr_prev>0, pgp3_prev>13.5) %>% nrow()
```
...
# count positives out of total
clu_1to5 %>% filter(location_year_name %in% lyn_1to5_list, pcr_n_tested>= min_clu_n, !is.na(pcr_prev)) %>% nrow()
clu_1to5 %>% filter(location_year_name %in% lyn_1to5_list, pcr_n_tested>= min_clu_n, pcr_prev>0) %>% nrow()
```
...
sp_fig <- get_sens_spec_fig(df = sens_spec_all,
  ...
  theme(plot.margin = margin(0,0,0,20,"pt"))
...
sis_fig <- get_sens_spec_fig(df = sens_spec_all,
                            lyn_list = "lyn_all_list",
                            dvar_input = "pcr_prev",
                            cutoff_var = "sis_foi",
...
roc_sir_fig <- sens_spec_all %>%
  ...
  theme_minimal() +
  theme(legend.position = "none",
        plot.margin = margin(0.2,0.2,0.45,0.2,unit = "cm"))
...
ggsave(filename = here("output", "fig6_sens_spec_sp.png"),
       sens_spec_fig,
...
ggsave(filename = here("output", "figS5_sens_spec_scr.png"),
       sens_spec_fig_scr,
       width = 8.5, height = 3.5, units = "in")
```

**Figure S6. Seroprevalence among children 1-5y**

```{r save figs6, fig.width = 8.5, fig.height = 3.5}
sens_spec_1to5_fig <- plot_grid(plot_grid(sp_fig_1to5, roc_sp_1to5_fig, nrow = 1, labels = 'auto'),
                           step3, rel_heights = c(1, 0.15), ncol = 1)

sens_spec_1to5_fig

ggsave(filename = here("output", "figS6_sens_spec_sp_1to5.png"),
       sens_spec_1to5_fig,
       width = 8.5, height = 3.5, units = "in")
```

## Save Supplementary information tables 2-5
...
* Seroprevalence threshold for 90% sensitivity: `r sens_spec_all %>% filter(dvar == "pcr_prev", dcutoff == 0, var == "pgp3_prev") %>% filter(sens>=0.9) %>% arrange(desc(cutoff)) %>% slice(1) %>% pull(cutoff)`, specficity: `r sens_spec_all %>% filter(dvar == "pcr_prev", dcutoff == 0, var == "pgp3_prev") %>% filter(sens>=0.9) %>% arrange(desc(cutoff)) %>% slice(1) %>% pull(spec) %>% round(2)`
* Seroconversion (no seroreversion) threshold for 90% sensitivity: `r sens_spec_all %>% filter(dvar == "pcr_prev", dcutoff == 0, var == "sir_foi") %>% filter(sens>=0.9) %>% arrange(desc(cutoff)) %>% slice(1) %>% pull(cutoff)`, specficity: `r sens_spec_all %>% filter(dvar == "pcr_prev", dcutoff == 0, var == "sir_foi") %>% filter(sens>=0.9) %>% arrange(desc(cutoff)) %>% slice(1) %>% pull(spec) %>% round(2)`
* Seroprevalence threshold for 80% sensitivity: `r sens_spec_all %>% filter(dvar == "pcr_prev", dcutoff == 0, var == "pgp3_prev") %>% filter(sens>=0.8) %>% arrange(desc(cutoff)) %>% slice(1) %>% pull(cutoff)`, specficity: `r sens_spec_all %>% filter(dvar == "pcr_prev", dcutoff == 0, var == "pgp3_prev") %>% filter(sens>=0.8) %>% arrange(desc(cutoff)) %>% slice(1) %>% pull(spec) %>% round(2)`
