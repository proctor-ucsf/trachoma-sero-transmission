...
get_sir_results <- function(df){
  ...
  temp_df <- df %>% filter(!is.na(pgp3_bin))
  
  glm_fit <- tryCatch(glm(pgp3_bin~1, offset = log(age_years),
                          data = temp_df,
...
get_sis_results <- function(df, curr_sr){
  ...
  temp_df <- df %>% 
    filter(!is.na(pgp3_bin)) %>% 
    group_by(age_years) %>%
    summarise(pgp3_pos = sum(pgp3_bin),
              n = n(),
              .groups = "drop") %>% 
...
get_gam_results <- function(df){
  ...
  temp_fit <- tryCatch(mgcv::gam(pgp3_bin~s(age_years, bs = "cr", k = 3),
                           data = temp_df,
                           family = binomial(link = "cloglog")),
